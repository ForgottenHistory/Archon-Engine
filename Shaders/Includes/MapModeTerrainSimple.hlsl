...
float4 RenderTerrainInternal(uint provinceID, float2 uv, float3 positionWS)
{
    ...
    // Simple approach: Sample terrain color directly from terrain.png
    // No need for index lookups or palettes - the texture already has the colors we want
    float2 correctedUV = float2(uv.x, 1.0 - uv.y);
    float3 terrainColor = SAMPLE_TEXTURE2D(_ProvinceTerrainTexture, sampler_ProvinceTerrainTexture, correctedUV).rgb;
