...
namespace Core.Loaders
{
    ...
    public static class ReligionLoader
    {
        ...
        public static void LoadReligions(Registry<ReligionData> religionRegistry, string dataPath)
        {
            ...
            {
                DominionLogger.LogWarning($"Religions directory not found: {religionsPath}");
                CreateDefaultReligions(religionRegistry);
                return;
            }

            var religionFiles = Directory.GetFiles(religionsPath, "*.txt");
            DominionLogger.Log($"ReligionLoader: Found {religionFiles.Length} religion files in {religionsPath}");
            ...
            foreach (var file in religionFiles)
            {
                ...
                {
                    DominionLogger.LogError($"ReligionLoader: Failed to load {file}: {e.Message}");
                }
            }

            DominionLogger.Log($"ReligionLoader: Loaded {loaded}/{religionFiles.Length} religion files, {religionRegistry.Count} religions registered");

            // If no religions loaded, create defaults
            if (religionRegistry.Count == 0)
            {
                DominionLogger.LogWarning("ReligionLoader: No religions loaded, creating defaults");
                CreateDefaultReligions(religionRegistry);
            }
        }
        ...
        private static void CreateDefaultReligions(Registry<ReligionData> religionRegistry)
        {
            ...
            DominionLogger.Log($"ReligionLoader: Created {defaultReligions.Length} default religions");
        }
    }
}