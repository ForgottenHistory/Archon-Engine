...
{
    /// <summary>
    /// Processes GAME layer commands (ICommand) with network synchronization.
    /// Complements CommandProcessor (which handles IProvinceCommand for ENGINE layer).
    ///
    /// ARCHITECTURE:
    /// - IProvinceCommand (ENGINE) → CommandProcessor → province state changes
    /// - ICommand (GAME) → GameCommandProcessor → gameplay state changes
    ///
    /// Both use the same network bridge for multiplayer synchronization.
    ...
    /// </summary>
    public class GameCommandProcessor : IDisposable
    {
        ...
        public GameCommandProcessor(GameState gameState)
        {
        ...
        public void RegisterCommandType<T>() where T : ICommand, new()
        {
            ...
            {
                ArchonLogger.LogWarning($"GameCommandProcessor: Command type {type.Name} already registered", "core_commands");
                return;
            }
            ...
            ArchonLogger.Log($"GameCommandProcessor: Registered {type.Name} as type ID {typeId}", "core_commands");
        }
        ...
        public void SetNetworkBridge(INetworkBridge bridge)
        {
            ...
            {
                networkBridge.OnCommandReceived += HandleRemoteCommand;
                ArchonLogger.Log("GameCommandProcessor: Network bridge attached", "core_commands");
            }
        }
        ...
        public bool SubmitCommand<T>(T command, out string resultMessage) where T : ICommand
        {
            if (disposed)
            {
                resultMessage = "GameCommandProcessor is disposed";
                return false;
            }
            ...
            if (!commandTypeIds.TryGetValue(commandType, out ushort typeId))
            {
                ...
                if (IsMultiplayer)
                {
                    ArchonLogger.LogWarning($"GameCommandProcessor: Command type {commandType.Name} not registered for networking - executing locally only!", "core_commands");
                }
                return ExecuteLocally(command, out resultMessage);
            }

            ArchonLogger.Log($"GameCommandProcessor: Submitting {commandType.Name} (typeId={typeId}, multiplayer={IsMultiplayer}, authoritative={IsAuthoritative})", "core_commands");
            ...
            {
                byte[] commandData = SerializeCommand(typeId, command);
                ArchonLogger.Log($"GameCommandProcessor: Broadcasting {commandType.Name} ({commandData.Length} bytes) to clients", "core_commands");
        ...
        private bool ExecuteLocally<T>(T command, out string resultMessage) where T : ICommand
        {
            ...
            catch (Exception e)
            {
                ...
                ArchonLogger.LogError($"GameCommandProcessor: Command execution failed - {e.Message}", "core_commands");
        ...
        {
            ArchonLogger.Log($"GameCommandProcessor: HandleRemoteCommand from peer {peerId} ({commandData?.Length ?? 0} bytes)", "core_commands");

            if (commandData == null || commandData.Length < 2)
            {
                ArchonLogger.LogWarning($"GameCommandProcessor: Invalid command data from peer {peerId}", "core_commands");
                return;
            }
            ...
            if (!commandFactories.TryGetValue(typeId, out var factory))
            {
                ...
                ArchonLogger.Log($"GameCommandProcessor: TypeId {typeId} not registered, ignoring", "core_commands");
                return;
            }
            ...
            try
            {
                ...
                if (IsAuthoritative)
                {
                    ...
                    if (command.Validate(gameState))
                    {
                        ...
                        ArchonLogger.Log($"GameCommandProcessor: Executed and broadcast command from peer {peerId}", "core_commands");
                    }
                    else
                    {
                        ArchonLogger.LogWarning($"GameCommandProcessor: Rejected invalid command from peer {peerId}", "core_commands");
                    }
                }
                else
                {
                    ...
                    command.Execute(gameState);
                    ArchonLogger.Log($"GameCommandProcessor: Executed command from host", "core_commands");
                }
            }
            catch (Exception e)
            {
                ArchonLogger.LogError($"GameCommandProcessor: Failed to process remote command - {e.Message}", "core_commands");
            }
        }
