    ...
    public class CommandBuffer : IDisposable
    {
        ...
        public CommandBufferResult AddConfirmedCommand(IProvinceCommand command, uint serverTick)
        {
            ...
            try
            {
                ...
                {
                    DominionLogger.LogWarning($"Received command for tick {serverTick} but already confirmed up to {confirmedTick}");
        ...
        private CommandBufferResult PerformRollback(uint rollbackToTick)
        {
            try
            {
                ...
                DominionLogger.Log($"Performed rollback to tick {rollbackToTick}, re-simulated {predictedTick - rollbackToTick} frames");
                return CommandBufferResult.CreateSuccess();
            }
            catch (Exception ex)
            {
        ...
        private void SaveStateSnapshot()
        {
            ...
            {
                DominionLogger.LogError($"Failed to save state snapshot: {ex}");
            }
        }
        ...
        private void RestoreStateSnapshot(StateSnapshot snapshot)
        {
            ...
            DominionLogger.LogWarning("RestoreStateSnapshot is not fully implemented - would restore complete simulation state");
