    ...
    public class ProvinceMapLoaderTests
    {
        ...
        public void LoadProvinceMap_ValidFile_ShouldSucceed()
        {
            ...
            try
            {
                ...
                ArchonLogger.Log($"Loaded {result.ProvinceCount} provinces from {result.Width}x{result.Height} map");
            }
            finally
            {
                result.Dispose();
        ...
        public void LoadProvinceMap_LargeMap_ShouldHandlePerformance()
        {
            ...
            try
            {
                if (result.Success)
                {
                    ArchonLogger.Log($"Loading {result.ProvinceCount} provinces took {stopwatch.ElapsedMilliseconds}ms");
