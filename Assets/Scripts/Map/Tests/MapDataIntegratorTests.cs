    ...
    public class MapDataIntegratorTests
    {
            ...
            try
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        Debug.Log($"Integration test successful:");
                        Debug.Log($"  - Loaded {loadResult.ProvinceCount} provinces");
                        Debug.Log($"  - Found {neighborResult.TotalNeighborPairs} neighbor pairs");
                        Debug.Log($"  - Generated metadata for {metadataResult.ProvinceMetadata.Count} provinces");
                    }
                    finally
                    {
        ...
        public void IntegratedMapLoading_Performance_ShouldCompleteReasonably()
        {
            ...
            try
            {
                if (result.Success)
                {
                    Debug.Log($"Complete map loading took {stopwatch.ElapsedMilliseconds}ms for {result.ProvinceCount} provinces");
                    ...
                    Debug.Log($"Estimated memory impact: {(memoryAfter - memoryBefore) / (1024 * 1024)}MB");
                }
            }
            finally
            {
        ...
        public void LoadProvinceMap_DataConsistency_ShouldHaveConsistentData()
        {
            ...
            try
            {
                ...
                Debug.Log($"Data consistency checks passed: {validPixels} valid province pixels");
            }
            finally
            {
                result.Dispose();
        ...
        public void LoadProvinceMap_MemoryManagement_ShouldCleanupProperly()
        {
            ...
            Debug.Log("Memory management test completed - multiple load/dispose cycles successful");
        }
        ...
        public void LoadProvinceMap_ErrorHandling_ShouldHandlePartialFailures()
        {
            ...
            try
            {
                ...
                if (!result.Success)
                {
                    ...
                    Debug.Log($"Expected partial failure: {result.ErrorMessage}");
                }
            ...
            if (File.Exists(provincesPath))
            {
                ...
                try
                {
                    if (result.Success)
                    {
                        ...
                        Debug.Log($"Found {smallProvinces} very small provinces (< 5 pixels) out of {provincePixelCounts.Count} total");

                        // This is acceptable but worth noting for performance
                        if (smallProvinces > provincePixelCounts.Count * 0.1f)
                        {
                            Debug.LogWarning($"High ratio of small provinces: {smallProvinces}/{provincePixelCounts.Count}");
                        }
                    }
                }
                finally
