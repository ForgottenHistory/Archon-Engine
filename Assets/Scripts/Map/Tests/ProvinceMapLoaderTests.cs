    ...
    public class ProvinceMapLoaderTests
    {
        ...
        public void LoadProvinceMap_ValidFile_ShouldSucceed()
        {
            ...
            try
            {
                ...
                DominionLogger.Log($"Loaded {result.ProvinceCount} provinces from {result.Width}x{result.Height} map");
            }
            finally
            {
                result.Dispose();
        ...
        public void LoadProvinceMap_LargeMap_ShouldHandlePerformance()
        {
            ...
            try
            {
                if (result.Success)
                {
                    DominionLogger.Log($"Loading {result.ProvinceCount} provinces took {stopwatch.ElapsedMilliseconds}ms");
