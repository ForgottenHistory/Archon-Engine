...
namespace Map.Simulation.Commands
{
    /// <summary>
    ...
    public class CommandBuffer : IDisposable
    {
        ...
        public CommandBufferResult AddConfirmedCommand(IProvinceCommand command, uint serverTick)
        {
            ...
            try
            {
                ...
                {
                    Debug.LogWarning($"Received command for tick {serverTick} but already confirmed up to {confirmedTick}");
        ...
        private CommandBufferResult PerformRollback(uint rollbackToTick)
        {
            try
            {
                ...
                Debug.Log($"Performed rollback to tick {rollbackToTick}, re-simulated {predictedTick - rollbackToTick} frames");
                return CommandBufferResult.CreateSuccess();
            }
            catch (Exception ex)
            {
        ...
        private void SaveStateSnapshot()
        {
            ...
            {
                Debug.LogError($"Failed to save state snapshot: {ex}");
            }
        }
        ...
        private void RestoreStateSnapshot(StateSnapshot snapshot)
        {
            ...
            Debug.LogWarning("RestoreStateSnapshot is not fully implemented - would restore complete simulation state");
