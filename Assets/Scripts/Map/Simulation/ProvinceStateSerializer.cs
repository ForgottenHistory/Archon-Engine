...
namespace Map.Simulation
{
    ...
    public static class ProvinceStateSerializer
    {
        ...
        public static bool DeserializeFullState(byte[] data, ProvinceSimulation simulation, out string errorMessage)
        {
            ...
            unsafe
            {
                fixed (byte* ptr = data)
                {
                    ...
                    Debug.Log($"Successfully deserialized {provinceCount} provinces (version {stateVersion})");
                    return true;
                }
            }
        }
        ...
        public static byte[] SerializeDeltaState(ProvinceSimulation simulation)
        {
            ...
            Debug.Log($"Serialized delta state: {dirtyCount} changed provinces, {totalSize} bytes");
            return buffer;
        }
